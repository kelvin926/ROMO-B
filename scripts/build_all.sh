...
source_ros_setup "$repo_root/scripts/source_env.sh"
...
if [[ "$project_only" == false ]]; then
  ...
  source_ros_setup "$repo_root/vendor_ws/install/local_setup.bash"
fi
...
autoware_internal_planning_msgs autoware_map_msgs autoware_planning_msgs \
...
printf 'Build complete. Source %s/scripts/source_env.sh\n' "$repo_root"
